...
2. **Quiet hours are compressed, not cut.** The scale runs faster through
   ...
   ONLY WHERE THERE IS A BUSY PART TO GIVE THE PAPER TO. A moment takes no
   time and stretches none; a pause under an hour between two things is not
   a quiet stretch; and a day whose busy part is under a quarter of the
   window is drawn linear. On a day of moments the hatch covered most of the
   strip and read as the scale's normal rate: "why are there speed marks if
   it's just like normal?"

   A quiet run of six hours or more (a night) is always compressed, and the
   view's hours cap counts it at its compressed rate, so an evening board
   reaches tomorrow's things instead of drawing the empty morning.
...
2e. **How much of the day is on the board is the DRAWING's question.** The
   ...
   the clock alone, so two renders in the same step lay out the same board.

   Nothing reaches back past the opening except the LEAD. Every event still
   running at the lookback used to widen the window so that it could not be
   cut, and that undid the step: a school day from half past eight held the
   four o'clock window open at half past eight, and a board read in the
   evening still opened in the morning with eleven hours of spent day on it.

   WHERE IT CLOSES is the first of: the hours the view can hold, the events it
   can hold, or a gap with nothing in it longer than about three hours -- the
   length at which a gap stops reading as "later today" and starts reading as
   another day. Unless what is this side of that gap is thinner than a board,
   in which case it crosses: at half past ten at night everything left is on
   the far side of it, and a board that stopped at the gap would be blank.

   Counted from the OPENING, twenty-four hours of span meant the later it got
   the further the board reached into tomorrow, so trimming the morning bought
   a night. A board with seven things still on it borrowed Tuesday to draw it.
2f. **Room in front of the first thing drawn.** The line names are drawn inside
   the map at the leading edge (rule 33e), and one name is about an hour wide,
   so an event sitting on the opening has its shelf and its spur drawn in the
   legend. Two hours, and it moves the opening back by at most that much,
   because the event it moves for is at or after the opening it started from.
2g. **An event that straddles an edge is drawn FROM the edge.** Clipped, with
   the start dot's leading half off the paper: a whole dot would say it began
   there, which is the one thing this map exists not to get wrong, and no mark
   at all drops the question. A convergence already running when the board
   opens gets no diamond at the edge either, for the same reason.

   The half mark IS the line's head there: it takes the terminal slash's
   place, and a corridor already running gets the diamond cut in half across
   both rails instead of two slashes. Drawn on the same point the two came
   out as one scribble. And the room in front of the first event (2f) may not
   land the edge ON another event's start: at ten past two it put the
   opening on the school day's half past eight. The edge goes just past that
   start instead, so the event straddles it.
   ...
   entirely: no corridor could be routed from there, the two rails stayed on
...
2b. **The night is not drawn.** It was, for a long time: ten at night to
   ...
   The marks nearest either end of a stretch are shorter: the scale ramps up
   into it and down out of it.
...
2b-iii. **Rails cross the midnight whole.** A rail with nothing on it at the
   ...
   kept to its own day.
2b-iv. **A line crossing a shared event's bar does not call there**: it
   bridges the bar or the bar goes under it (2b-xi), and it wears no ring.
2b-v. **A shared event's bar is hollow**: an ink outline with paper inside,
   fatter than any rail, so it is never read as somebody's line.
2b-vi. **Back to back is one bar.** A shared event starting within a few
   rings of the bar before it, for the same people or a group inside theirs,
   joins that bar: one set of rings across everybody, and one branch off it
   running to where the last of them ends, each a stop on it with its own
   dot, tick and name; each long one's people are ticked where it ends. Two
   people at a long thing still run their lines together (rule 21).
2b-vii. **A shared calendar's line is a ladder.** Where the config names its
   people, a line that is not one of them (a "Family" calendar) is hollow
   with rungs, a style no person gets; the people keep theirs.
2b-viii. **A mark is made the way its rail is made.** An end tick and a
   terminal slash wear the rail's outer weight with the rail's own paper core
   inside: a hollow rail ends hollow, a dotted one dotted. A shared event's
   branch is hollow at the bar's width. Every line starts with a slash, or
   with dots where it had something before the board opened.
2b-ix. **Ring letters tell people apart.** Where names share a first letter,
   the second letter is the first one along where the names differ (Maggie
   Mg, Marge Mr).
2b-x. **A name may stand across its own person's bar.** Where the words
   cover their own stop, a shared event's bar that person is on tunnels under
   them: paper across the bar for the rows of the words. The rings stay
   clear, and nobody else's bar may be written on.
2b-xii. **A mark is full or hollow**, never dotted or dashed: a patterned
...
2b-xiii. **Only the night runs fast.** The scale compresses only from
   midnight to a day's first event and from its last event to midnight (the
   hour before the first event kept at full rate), and nothing is drawn for
   it: no hatch on the strip, no speed marks on the rails.
2b-xiv. **Several things at one stop are a list.** A stop made of several
   names (a calendar's `mergeSameTime`) is captioned a name to a row over
   its time; where that does not fit, two to a row; then the first name and
   how many more ("Restafval +3"). A name is never cut in the middle. A
...
2c. **Each day names itself on the hour strip, and the strip is the
   header.** The day the board is on is a panel in the framework's
   `inverse`; the day it reaches into is a light wash beside it, with a gap
   of paper at the midnight between them. An hour label says only the time, and "09:00" on a 36-hour
...
2h. **A full view says Now and Next in words.** In the title band of today's
    panel (tomorrow's, when today is a sliver): what is on, what comes next
    with its start time, and whose. The strip keeps a row for it on a full
    view; it is cut at a word, then loses the Now line, then is left off.
...
25. **A shared event two people are at is a convergence.** The lines lean in,
    run together for the length of it, and part again. It is the best drawing
    on this map and it is for TWO: with three it is three rails deep, which
    costs every one of them their own row for hours, puts the merge diamond
    across however far apart the outermost two happen to be, and leaves the
    names at each end in a stack nobody can pair with a rail -- because inside
    a corridor there is no rail to pair them with.
25a. **Wider than two is a tie.** A bar across the lines that are in it, a ring
    on each, a tick on every one of them where it ends, and nobody leaves
    their row. It says the same thing in one place and it scales, which the
    corridor does not. The name hangs off the bar (rule 31), not off its
    owner's rail: given a shelf as well, a six-hour school day was drawn twice
    over, once as the bar across three people and once as one person's branch.
26. **A convergence falls back to the interchange tie** when the lines cannot
    all be free at that minute -- one of them is already converging
    elsewhere. A siding is not a reason: being on Spring Break has never
    stopped anybody having dinner.
    The fallback used to be a BUNDLE, a comb of rails on its own minute. It
    was removed on 2026-09-13 after being found unreachable: every shared
    event with a span is route-shared by the time anything is drawn, its
    planner had no caller, and its drawing function returned on its first
    line -- so an event that did reach it would have had no rails AND no
    tie. The tie is what these have actually been drawn with all along.
...
29e. **A start is a dot, an end is a tick, and a tick is drawn at the
    corner, upright.**
    ...
    second line crossing it.
    A tick stands SQUARE to the rail (rule 29) wherever there is one rail to
    be square to. At a corner there are two, and on a 90 square means a bar
    lying flat in open paper with the line running past it -- so in both of
    those the tick is drawn UPRIGHT instead, across the axis, at the minute
    the thing ended. Upright is what reads against a time axis anyway.
    THIS USED TO SAY "a tick needs level rail" and drew nothing at all where
    the end landed on a fillet or a transition. That was written from the 90
    case and it cost every tick on a turn: with the trunk leaning through an
    event (rule 8) the climb ends exactly on a corner, so the end of Swim
    Training was not marked at all -- the one place the vocabulary broke.
    An end that is not drawn is worse than an end drawn upright.

29c. **A line ends on a 45 slash** at the far edge. Its own mark, not the
    crossbar an event's end tick uses: three different things drawn with one
    glyph can only be told apart by position. Nothing else on this map is
    drawn at 45, which is what makes it read. At the leading edge the rail is
    flush: that edge is where the window opens, not where a day starts. An
    event's end tick is drawn at its rail's own weight, square-ended.
...
34h. **The time text sits as close to the start dot as possible.**
    That is the whole rule; everything else about a caption's two rows falls
    out of it. The time is the one row that ties a caption to a minute, so of
    the two it is the row that has to be beside the minute, and where the
    caption sits decides which row and which end that is.
    Beside or below what it names, the near row is the top one: time over
    title, as it has always been.
    In the wedge a climb opens (rule 8) the caption sits above the flat run
    and ends where the slope starts, so the near row is the BOTTOM one and
    the near end is the RIGHT one. Both turn over: the time goes under the
    title, the rows are ranged right so they finish together at the corner,
    and the box's right edge stands directly over the dot -- the words are a
    rail's clearance above the flat run and the slope rises away to the right
    of the turn, so nothing is ever closer to the rail for it.
    Both rows are the same two rows and the box is the same size, so nothing
    placed against it has to know.
...
38a. **A caption's time is in bold black: when it starts, and for anything
    of an hour or more when it ends too** ("8:30am – 3pm", "4 – 5:30pm"). The strip's
    words go a size up with the titles on a full view. On a full
    view its title is a size up (`text--large`), stepping back to the base
    size before it gives up its time. A half or a quadrant keeps the base
    size.

38b. **A shared event's rings carry initials** on a full view: each ring the
...
38b. **REMOVED WITH THE CARS.** This said that the letter inside a car had to
    be distinguishable from every other, and grew it a character at a time
    until it was -- Marge and Maggie both came out `M`. The board does not
    draw cars any anymore, so there is no letter to be right about, and the
    configured `badge` that could pin one is gone from CONFIG.md with it.

38c. **Rules 39 to 42 are `solver/order.js`, not the payload.** They used to
    be decided in `transform.js` and sent as a `line_offset` per line, which
    is a position, decided once, by the one thing on the board that cannot see
    the paper it is drawn on. What travels now is what the household said --
    the order they listed their people in, a pinned side, a colour -- and the
    arrangement is worked out where the events and the panel both are.
...
54. **It is declared at the line's head**, in a second row under the name,
    ...
    is.
    A state that starts on a later day the board draws is declared at the
    far head instead, where that day is ("Away in Leeds · Tue").
54a. **One person's seven hours or more in one place is a band**: a desk
    booking, working from home, a shift. A light rounded band lies behind the
    rail for those hours, following it, and is named once, small, along it
    where nothing leaves through the name. It draws no shelf, so the short
    things inside it branch off the one rail and visibly leave the band.
    Shared long events stay corridors and ties.
55a. **A line with more of its day past an edge ends in "...".** Something
    of that person's the same day ends before the board opens, or starts
    after it closes, so the slash that says the line stops here would be
    wrong: the rail ends in three dots at that edge instead. An all-day
    state's chevron (55) says more and wins where both apply.
55. **Both ends of that line become open chevrons.** A terminus slash says
    ...
    not begin at the first hour on the scale. Only the ends whose day the
    state covers: tomorrow's leave leaves today's end a slash.
...
58. **A half or a quadrant names each line once**, at its leading end. Both
    ends is for a panel read across the room; on a slot the second name is
    paper the captions need. The template says which view it is.
...
- **13 between two abutting holds.** Where a corridor ends on the same minute
  ...
  defect from the other side. Two ways of buying the turn some axis have been
  tried and measured, and neither landed.
...
- **An event during a diagonal run** should be a mark on the run, not a
  branch drawn beside it. Tried once, reverted. Rule 24 makes it matter more:
  where the gate leaves no 45, the branch takes a right angle, and a mark on
  the diagonal would be the better drawing.
- **A long event** should arguably be a band along the main line rather than a
  siding that moves it, becoming a branch only when a shared event needs the
  line. Rules 9 to 11 and 26 describe the siding model that is there now.
