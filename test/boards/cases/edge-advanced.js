  ...
  for (const f of withEdge) {
    for (const v of VIEWS) {
      test('at an edge ring the dots stand between it and the track: ' + f.name + '/' + v, () => {
        ...
        // side there is. Where the legend has a column, the connector itself
        // stands at the paper's edge -- the meeting happened before the board
        // opened, and the column is that time -- and what runs from it to the
        // first minute is a dotted APPROACH rather than three dots: the line
        // arriving out of the time the board does not show.
        ...
        for (const r of rings) {
          ...
          if (column) {
            ...
            if (cx > e0 + r.w) bad.push(r.owner + ': its connector is at ' + Math.round(cx)
              + ', not at the paper\'s edge ' + Math.round(e0));
            const lead = rep.rects.filter((q) => q.role === 'terminal-more' && q.owner === r.owner)[0];
            if (!lead) { bad.push(r.owner + ' has no approach from its connector'); continue; }
            const l0 = horiz ? Math.min(lead.ends[0][0], lead.ends[1][0]) : Math.min(lead.ends[0][1], lead.ends[1][1]);
            const l1 = horiz ? Math.max(lead.ends[0][0], lead.ends[1][0]) : Math.max(lead.ends[0][1], lead.ends[1][1]);
            if (l0 < cx) bad.push(r.owner + ': its approach starts at ' + Math.round(l0) + ', behind its own ring');
            if (Math.abs(l1 - a0) > 1.5) bad.push(r.owner + ': its approach stops at ' + Math.round(l1)
              + ', not at the first minute ' + Math.round(a0));
            if (!/\d/.test(lead.dash || '')) bad.push(r.owner + ': its approach is not dotted');
            assert(!bad.length, bad.slice(0, 4).join('; '));
            continue;
          }
