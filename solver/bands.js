...
function boardFor(spec, st) {
  ...
  spec.wants.forEach(function (w, wi) {
    ...
    // ...AND NEVER MORE OF THE CLOCK THAN THE EVENT ITSELF OWNS.
    //
    // This run is what puts the dot on flat shelf instead of on the bend the
    // spur has just turned, so it is a corner's worth plus a mark's. It is
    // also a FLAT amount, which on a long meeting is a quarter of it and on a
    // short one is all of it: "Pick Up Kids", twenty-four units of afternoon,
    // was being approached over twenty-five. The eye reads the whole shape as
    // the event, so a quarter of an hour drawn like that is an hour of
    // somebody's day -- "that cpoe one now looks like it's busy but it's
    // not".
    //
    // Held to three quarters of the event where the event is short, and never
    // below a mark's own radius, which is where the dot would start sitting on
    // the curve rather than past it. A long event does not notice; a short one
    // stops announcing itself from a minute it has nothing to do with.
    //
    // Three quarters and not a half: the run is also the paper the caption
    // hangs off, and at a half one of two back-to-back shared events lost its
    // name entirely (test/boards/cases/shared.js). A shelf that is shorter
    // than the words it carries has solved the wrong problem.
    var pre = spec.corner * 0.6 + spec.markR;
    pre = Math.min(pre, Math.max(spec.markR, (w.a1 - w.a0) * 0.75));
  ...
  function cutsHead(l, routed) {
    return (spec.fixed || []).some(function (fx) {
      ...
      // `tl` came from `b.lineByKey`, which only ever returns something
      // already in `b.lines` -- the `indexOf` this used to run afterwards
      // to confirm that was a second O(lines) scan proving what the first
      // one already guaranteed, on every fixed terminus, on every shelf,
      // on every board.
  ...
  shelvesMade.forEach(function (sh) {
    ...
    // ...and a spur through its OWN line's name is moved too: the word that
    // says whose rail this is, with the rail's own branch through it.
  ...
  // EVERY LINE'S NAMES, BY LINE. Deciding where one name goes asks, for each
  // rail on the board, how tall THAT rail's name is at this end -- which was
  // a scan of all the board's fixed ink, inside a loop over the lines, inside
  // a loop over the names. Three deep on a seven-line board is ten million
  // comparisons over a hard solve, and every one of them is looking for one
  // of two entries. They are indexed here instead, in the order they would
  // have been found, so the last match still wins.
  var headsOf = {};
  headFx.forEach(function (g) {
    if (g.kind !== 'terminus') return;
    (headsOf[g.line] = headsOf[g.line] || []).push(g);
  });
  headFx.forEach(function (fx) {
    ...
    if (fx.kind === 'terminus') {
      ...
      b.lines.forEach(function (o) {
        ...
        var oh = nh, mine = headsOf[o.key];
        if (mine) for (var mi = 0; mi < mine.length; mi++) {
          var g = mine[mi];
          if (Math.abs((g.at == null ? spec.axis.a1 : g.at) - atA) < 1) oh = headH(g);
        }
      ...
      if (cutters.length) {
        ...
        // Up to TWICE the name's own width. At one width a name whose branch
        // leaves the rail well in from the edge had nowhere to go and was cut
        // instead, which is the worse of the two: a name a little way in from
        // the end of its rail still reads as that rail's, a name with a branch
        // through it reads as nothing.
        var wdt = (f.a1 - f.a0) * 2, gap = spec.railGap;
  ...
  // BOUNDS FIRST, THE SEGMENTS ONLY WHERE THEY COULD MEET. Every branch was
  // tested against every other line's every segment regardless of where
  // either one is, on every board a candidate move is priced against -- most
  // pairs are nowhere near each other in time or level, and a box that does
  // not overlap the other box cannot hold a crossing segment either. `bounds`
  // is the same memoised box `lineTouches` already trusts for this (board.js).
  b.lines.forEach(function (l) {
    ...
    b.lines.forEach(function (o) {
      ...
      var ob = o.bounds();
      if (lb.a1 < ob.a0 || ob.a1 < lb.a0 || lb.c1 < ob.c0 || ob.c1 < lb.c0) return;
...
function solveBands(spec, opts) {
  ...
  // gaps/steps/forms are typed arrays (see `start`), so `.slice()` here is a
  // memcpy rather than a boxed-number copy, and it is now called only when a
  // move is actually kept (see `forEachMove`), not once per candidate tried.
  function clone(x) {
  ...
  function troubled(st) {
    ...
    // THE SAME CACHE `trial` READS. This call was left out of it, so once a
    // pass, every caption's candidate positions were rebuilt from nothing
    // even though most of them are the same geometry `trial` just cached
    // for another candidate a moment ago -- and this call never seeded a
    // single entry for `trial` to reuse either. The cache is a pure lookup
  ...
  // THE MOVES, OFFERED ONE AT A TIME RATHER THAN BUILT AS A LIST.
  //
  // This used to clone `st` -- three fresh arrays -- for every candidate
  // before any of them was priced: every pair of gaps at three shifts, plus
  // a clone per form and per step move. Almost all of them were thrown away
  // unscored (`opts.evals` stops the descent long before a full
  // neighbourhood is tried), so the clones a busy board never looked at
  // were most of the traffic a profile found in the garbage collector.
  //
  // `forEachMove` instead mutates `st` IN PLACE for one candidate, calls
  // `onMove` with it already holding that move, and then undoes exactly
  // that mutation before trying the next -- so `st` is byte-for-byte what
  // it was before this function ran whenever nothing in the pass is kept,
  // and `score` is handed the real state rather than a fresh object built
  // to be read once. `onMove` returns true to stop early (the pool or the
  // deadline ran out); every loop below carries that flag so a stop found
  // deep in the gap search does not go on to offer a single step or form.
  //
  // The ORDER every move is offered in is unchanged -- see the note at
  // bands.js:1714 (now below `descend`) on why a reordering is a different
  // search, not a faster one.
  var SHIFTS = [40, 16, 6];
  var MODES = [0, 2, -2];
  function lineOf(key) {
  ...
  function forEachMove(st, onMove) {
    var i, j, k, stopped = false;
    var who = troubled(st);
    ...
    for (i = 0; i <= n && !stopped; i++) {
      for (j = 0; j <= n && !stopped; j++) {
        if (i === j) continue;
        for (k = 0; k < SHIFTS.length && !stopped; k++) {
          if (st.gaps[i] - SHIFTS[k] < minGap) continue;
          var gi = st.gaps[i], gj = st.gaps[j];
          st.gaps[i] = gi - SHIFTS[k]; st.gaps[j] = gj + SHIFTS[k];
          stopped = !!onMove();
          st.gaps[i] = gi; st.gaps[j] = gj;
        }
      }
    }
    ...
    for (var a = 0; a < spec.wants.length && !stopped; a++) {
      var w2 = spec.wants[a];
      if (inWho[a]) continue;
      ...
      for (var k4 = 0; k4 < (st.forms[a] || 0) && !stopped; k4++) {
        var of4 = st.forms[a];
        st.forms[a] = k4;
        stopped = !!onMove();
        st.forms[a] = of4;
      }
      if (stopped || w2.tie || w2.allDay || w2.pill) continue;
      var md3 = modesFor(a);
      for (var k3 = 1; k3 < md3.length && !stopped; k3++) {
        if (st.steps[a] === md3[k3]) continue;
        var os3 = st.steps[a];
        st.steps[a] = md3[k3];
        stopped = !!onMove();
        st.steps[a] = os3;
    ...
    for (var wi = 0; wi < who.length && !stopped; wi++) {
      var i2 = who[wi];
      // A tie's spur is a fact (above); the search has no move to offer it.
      if (!spec.wants[i2].tie) {
        var md2 = modesFor(i2);
        for (var k2 = 0; k2 < md2.length && !stopped; k2++) {
      ...
      for (var k2b = 0; k2b < spec.wants[i2].formCount() && !stopped; k2b++) {
        if (st.forms[i2] === k2b) continue;
        var of2 = st.forms[i2];
        st.forms[i2] = k2b;
        stopped = !!onMove();
        st.forms[i2] = of2;
    ...
    // budget left to unfold them one at a time. This one replaces the
    // WHOLE forms array, so it saves and restores all of it rather than one
    // index -- three times a pass, not once per candidate.
    for (k = 0; k < 3 && !stopped; k++) {
      var origForms = st.forms.slice();
      for (var fi2 = 0; fi2 < spec.wants.length; fi2++) {
        var fs = spec.wants[fi2].forms, val = fs.length - 1;
        for (var fi = 0; fi < fs.length; fi++) {
          var rg = fs[fi].rung != null ? fs[fi].rung : fi;
          if (rg >= k - 1e-9) { val = fi; break; }
        }
        st.forms[fi2] = val;
      }
      stopped = !!onMove();
      st.forms.set(origForms);
    }
  }
  ...
  function descend(st0) {
    ...
    for (var pass = 0; pass < (opts.sweeps || 40); pass++) {
      var best = null, bestS = cur;
      forEachMove(st, function () {
        var s2 = score(st); spent++;
        ...
        if (opts.pool && opts.pool.left != null && opts.pool.used >= opts.pool.left) { spent = budget; return true; }
        // `st` holds this candidate right now; clone it only when it is
        // actually kept, which is rare next to how many are tried.
        if (s2.cost < bestS.cost - 1e-9) { bestS = s2; best = clone(st); }
        ...
        if (opts.deadline && (spent & 15) === 0 && Date.now() > opts.deadline) { spent = budget; return true; }
        return spent >= budget;
      });
  ...
  function start(steps, forms) {
    var formsArr = new Int16Array(spec.wants.length);
    spec.wants.forEach(function (w, i) { formsArr[i] = forms === 0 && w.members ? crowdStart(w) : forms; });
    return { gaps: evenGaps(),
             steps: new Int16Array(spec.wants.length).fill(steps),
             forms: formsArr };
  }
...
function solve(spec, opts) {
  ...
  list.forEach(function (cand) {
    ...
    // A LINE'S NAME CUT BY ITS OWN BRANCH costs four mistakable captions.
    // Counted as a fault, the restarts gave up readable captions to keep the
    // legend clean; at one caption's price, a seven-line board still took the
    // cut with a clean answer a restart away -- the name that says whose rail
    // this is, with the rail's own branch through it, is worth more than that.
    // Somebody else's rail through a name is a fault as it always was.
