...
function fit(spec, opts) {
  ...
  // How much clock there was to begin with, for the guard below.
  var started = opts.deadline ? Math.max(0, opts.deadline - Date.now()) : 0;
...
// ---------------------------------------------------------------- the window
//
// LESS OF THE DAY, RATHER THAN A DAY WITH NO CLOCK ON IT -- BUILT, MEASURED,
// AND TAKEN BACK OUT. Kept as a note because the measurements are the useful
// part and the next person will have the same idea.
//
// A crowded board gives up its captions' time rows first, and asked to stop
// doing that -- "just show less hours until it can show the times again" --
// the obvious answer is to solve the board at a few lengths of day and keep
// whichever loses least, in the currency `fit` already uses. It works. On the
// reported board it turned 0 of 23 captions with their time into 18 of 18,
// shedding nothing, and over 216 households it took shed from 36 to 21 and
// people dropped from 30 to 21.
//
// IT NEEDS SIX SECONDS AND THE PANEL HAS THREE. Every one of those numbers
// was measured with the layout suite's clock, which is thirty seconds so that
// the eval pool and not the clock decides what a board looks like. The device
// gives the page three (`data-metro-solve-seconds`), and no suite here had
// ever rendered a board on that. Measured since, on the same eighteen
// fixtures at every view:
//
//   3000ms  no ladder   584 captions, 403 with their time, 12 shed
//   3000ms  ladder      558 captions, 371 with their time, 33 shed
//   3000ms  ladder+guard 573 captions, 393 with their time, 22 shed
//   30000ms no ladder   591 captions, 393 with their time,  3 shed
//   30000ms ladder      556 captions, 428 with their time,  3 shed
//
// The reported board takes the times back at 6000ms and not at 4500, which is
// more than the page could give it even if the whole five seconds were the
// solver's. What this ladder buys, it buys with search; asked for without
// enough, it takes the board apart -- the first rung spends the budget, the
// shorter windows are solved in the scraps, and they are compared as though
// they had not been.
//
// So it is a performance problem wearing a layout problem's clothes. Make the
// solve twice as fast and six seconds is three, and this comes back exactly
// as it was written. solver/bench.js and the note in track.md have the
// profile: the caption search is the larger half.
