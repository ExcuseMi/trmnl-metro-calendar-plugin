...
function draw(board, spec, ctx) {
  ...
  function headRun(ln, lpts) {
    if (!horizontal || ln.branchOf || lpts.length < 2) return null;
    if (!(unfinished[ln.key] || [])[0] || edgeTieAt[ln.key] != null || levelHead[ln.key]) return null;
    // (a day that is a slice of something longer keeps its open chevron)
    if ((openEnded[ln.key] || [])[0]) return null;
