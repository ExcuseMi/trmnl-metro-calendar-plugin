...
function draw(board, spec, ctx) {
  ...
  function chevron(w) { return { back: 4 * S * 1.6, out: Math.max(4 * S, w * 0.9) }; }
  board.lines.forEach(function (ln, li) {
    ...
    if (t) {
      ...
      // ...AND CLEAR OF AN ARROW AT EITHER END.
      var op = (openAt[ln.branchOf || ln.key] || [false, false]);
      if (!ln.branchOf && (op[0] || op[1]) && pts.length > 1) {
        var cv = chevron(w), keep = pts.map(function (q) { return [q[0], q[1]]; });
        var back = cv.back;
        if (op[0]) {
          var g0 = keep[0], g1 = keep[1], l0 = Math.hypot(g1[0] - g0[0], g1[1] - g0[1]);
          if (l0 > back) { keep[0] = [g0[0] + (g1[0] - g0[0]) * back / l0, g0[1] + (g1[1] - g0[1]) * back / l0]; }
        }
        if (op[1]) {
          var h1 = keep[keep.length - 1], h0 = keep[keep.length - 2];
          var l1 = Math.hypot(h1[0] - h0[0], h1[1] - h0[1]);
          if (l1 > back) { keep[keep.length - 1] = [h1[0] - (h1[0] - h0[0]) * back / l1, h1[1] - (h1[1] - h0[1]) * back / l1]; }
        }
        dOv = pathOf(keep, railR);
      }
  ...
  board.lines.forEach(function (ln) {
    ...
    [ln.pts[0], ln.pts[ln.pts.length - 1]].forEach(function (p, end) {
      ...
      if (open) {
        ...
        var dir = end ? 1 : -1, cvo = chevron(w), r2 = cvo.back, ahead = p[0] - dir * cvo.out;
        var t0 = xy(ahead - dir * r2, p[1] - r2), tip = xy(ahead, p[1]),
            t1 = xy(ahead - dir * r2, p[1] + r2);
