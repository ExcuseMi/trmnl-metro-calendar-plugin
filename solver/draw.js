...
var TRAIN_D = 'M814.817,382.75h-45.773c0-9.665-7.835-17.5-17.5-17.5h-57.5c-9.665,0-17.5,7.835-17.5,17.5h-64 c0-9.665-7.835-17.5-17.5-17.5h-150c-9.665,0-17.5,7.835-17.5,17.5h-28.058c-123.542,0-228.097,91.845-243.34,214.442 c-0.04,0.323-0.079,0.64-0.117,0.952c-0.166,1.379-0.064,2.719,0.241,3.986c-3.965,1.814-6.726,5.807-6.726,10.453v10.667 c0,6.351,5.149,11.5,11.5,11.5h193.611c6.351,0,11.5-5.149,11.5-11.5v-10.667c0-0.625-0.064-1.235-0.16-1.833h25.859 c-0.096,0.598-0.16,1.208-0.16,1.833v10.667c0,6.351,5.149,11.5,11.5,11.5h193.611c6.351,0,11.5-5.149,11.5-11.5v-10.667 c0-0.625-0.064-1.235-0.16-1.833h25.859c-0.096,0.598-0.16,1.208-0.16,1.833v10.667c0,6.351,5.149,11.5,11.5,11.5h193.611 c6.351,0,11.5-5.149,11.5-11.5v-10.667c0-0.625-0.064-1.235-0.16-1.833h0.16V418.389C850.456,398.706,834.5,382.75,814.817,382.75z M395.476,497.785c-17.829,23.27-45.942,38.094-77.072,38.094h-119.1c14.1-30.59,35.01-57.27,60.68-78.26h115.608 C396.287,457.62,408.062,481.358,395.476,497.785z M451.544,482.62c0-13.807,11.193-25,25-25h242c13.807,0,25,11.193,25,25v28.26 c0,13.807-11.193,25-25,25h-242c-13.807,0-25-11.193-25-25V482.62z M812.044,579.75h-352c-4.694,0-8.5-3.806-8.5-8.5 s3.806-8.5,8.5-8.5h352c4.694,0,8.5,3.806,8.5,8.5S816.738,579.75,812.044,579.75z M820.544,510.88c0,13.807-11.193,25-25,25h-1.5 c-13.807,0-25-11.193-25-25v-28.26c0-13.807,11.193-25,25-25h1.5c13.807,0,25,11.193,25,25V510.88z';
  ...
  (board.fixed || []).forEach(function (fx) {
    ...
    if (strip && horizontal) {
      ...
      // ...AND OUTLINED, because it straddles the edge of the panel. Paper
      // on ink reads; the half below the rail is paper on PAPER, so the
      // wheels and the nose simply were not there and the car looked bitten
      // off at the strip's edge ("train needs an outline"). The outline is
      // drawn in the icon's own units -- the path is inside the scale, so a
      // width in board pixels has to be divided back out -- and it takes
      // the windows with it, which is what a drawn train wants anyway.
      var tOut = Math.max(0.6, 0.5 * S);
      var tb = svgEl(doc, 'path', { d: TRAIN_D, 'stroke-width': tOut / tk,
    ...
    // ...AND AROUND THE SKY'S GLYPHS. The line crosses the sky ROW on
    // purpose -- it is the clock's row too -- but crossing the row is not
    // the same as going through a mark on it: at three minutes to midnight
    // it ran straight down the middle of the moon ("now line colliding with
    // weather icons"). The band stays crossed; the glyphs get the gap a
    // name gets.
    (board.fixed || []).forEach(function (g) {
      if ((g.kind !== 'sky' && g.kind !== 'weather') || !(g.a0 - 2 * S <= a && a <= g.a1 + 2 * S)) return;
      cutRuns(g.c0 - 2 * S, g.c1 + 2 * S);
    });
  ...
  function askClock() {
    ...
    dayCuts.forEach(function (cut) {
      if (cut > nowA && r[1] - 2 * S > cut - 10 * S) {
        // ...AND IT ENDS ON THE LINE IT NAMES. Backed off the midnight by a
        // clear ten pixels and then six more, the pill stood a half of its
        // own width from the mark it is about: at three minutes to midnight
        // the time was one side of the corner and the line the other ("now
        // time not above the now line"). The clock cannot straddle the
        // panel change -- a solid pill is knocked out of the ink and drawn
        // into the paper, so half of it would go -- but it can stop AT the
        // line, which is the nearest it can stand to what it means. The cut
        // is always within half a pill of now here, or this branch is not
        // taken at all.
        r = place(pill, nowA, cRow, 'right');
      }
    });
  ...
  function stripFixture(fx, pass) {
    ...
    if (fx.kind === 'date') {
      ...
      for (var fi = 0; fi < forms.length; fi++) {
        ...
        // ...AND NOT A SIZE SMALLER FOR STANDING NEXT TO A WORD. Beside
        // "Today" the date was set in `label--small`, one step under
        // everything else on the strip, inside a hairline outline: on the
        // wall "Di 22 Sep" was the one thing on the header nobody could
        // read. It keeps the outline -- the clock's solid pill stays the
        // only solid mark up there -- and takes the strip's own size, which
        // is what the hours beside it are set in. Where that no longer fits
        // the ladder below still has the plain forms to fall to.
  ...
  // THE CLOCK UNDER A NAME STEPS UP WITH IT, WHERE THE BOARD HAS THE ROOM.
  //
  // A caption's time is set two sizes under an xlarge name and read on the
  // wall as a footnote ("time labels in the captions could be a bit
  // larger"). It cannot be a form in the ladder: offered as the richest
  // tier it cost the sweep four captions and thirty-seven times, because a
  // taller row is depth and depth is what the time row itself is paid out
  // of (measure-dom's TIME_BIG_CLS). So it is taken HERE, after every
  // caption has its place, out of depth that is already free -- the box
  // grows by four pixels or so and nothing else moves.
  //
  // ALL OF THEM OR NONE. Whether a box can grow depends on which side of
  // its rail it sits: one below has paper under it, one above has the rail.
  // Upgrading each on its own merits put two clock sizes on one board,
  // which reads as a mistake rather than as a decision, so the board asks
  // for all of them and takes the step only if every one is free.
  (function bigClocks() {
    if (!(spec.wants && spec.wants.length) || !horizontal) return;
    var want = [];
    board.caps.forEach(function (cp) {
      if (!cp.el || !cp.rowCls || cp.rowCls.length < 2) return;
      var ti = -1;
      cp.rowCls.forEach(function (cl, i) { if (/metro-time-tag/.test(cl) && !/metro-inline/.test(cl)) ti = i; });
      // only under the size the complaint was about, and only a row of its
      // own: the time set beside the name shares the name's line
      if (ti < 1 || !/text--xlarge/.test(cp.rowCls[0] || '')) return;
      var row = cp.el.childNodes[ti];
      if (!row || !row.className || !/text--small/.test(row.className)) return;
      want.push({ cp: cp, row: row, was: row.className });
    });
    if (!want.length) return;
    // grown, in the board's own units: the element is the one the ruler
    // measured, so its height is asked rather than guessed
    //
    // ...DOWNWARD WHERE THERE IS FLOOR UNDER IT, UPWARD WHERE THERE IS NOT.
    // A caption on the last line ends ON the map's bottom edge, with the
    // headlines under it: growing down there crosses into the band, and one
    // such caption would have taken the step away from the whole board. Its
    // extra pixels go above the words instead, which is paper it already
    // stands on.
    var grew = want.map(function (w) {
      var h0 = w.cp.el.offsetHeight;
      w.row.className = w.was.replace('text--small', 'text--base');
      var dh = Math.max(0, w.cp.el.offsetHeight - h0);
      w.row.className = w.was;
      var b = w.cp.box();
      var down = { a0: b.a0, a1: b.a1, c0: b.c0, c1: b.c1 + dh, dh: dh, up: false };
      if (down.c1 <= spec.cross.c1 + 0.5) return down;
      return { a0: b.a0, a1: b.a1, c0: b.c0 - dh, c1: b.c1, dh: dh, up: true };
    });
    var CLEAR = B.CAP_CLEAR;
    var ok = grew.every(function (g, gi) {
      if (g.c1 > spec.cross.c1 + 0.5 || g.c0 < spec.cross.c0 - 0.5) return false;
      // clear of every other caption, grown or not: a pair that both grow
      // is asked at both their new depths
      for (var oi = 0; oi < board.caps.length; oi++) {
        var other = board.caps[oi];
        if (other === want[gi].cp) continue;
        var wi = -1;
        want.forEach(function (w, k) { if (w.cp === other) wi = k; });
        var ob = wi >= 0 ? grew[wi] : other.box();
        if (g.a0 - CLEAR < ob.a1 && ob.a0 < g.a1 + CLEAR
            && g.c0 - CLEAR < ob.c1 && ob.c0 < g.c1 + CLEAR) return false;
      }
      // ...and of every rail, which is what a caption above its line runs
      // into as it grows downward
    ...
    want.forEach(function (w, wi) {
      w.row.className = w.was.replace('text--small', 'text--base');
      // the box that was tested is the box that is drawn: one grown upward
      // starts that much higher on the paper
      if (grew[wi].up && grew[wi].dh) {
        var t = parseFloat(w.cp.el.style.top) || 0;
        w.cp.el.style.top = (t - grew[wi].dh) + 'px';
        w.cp.c -= grew[wi].dh;
      }
      w.cp.h += grew[wi].dh;
      w.cp.rowCls = w.cp.rowCls.map(function (cl, i) {
        return i === w.cp.rowCls.indexOf(w.was) ? w.row.className : cl;
      });
    });
  })();
