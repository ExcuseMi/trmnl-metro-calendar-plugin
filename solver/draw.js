  ...
  if (hoursFx && spec.scale && spec.metro && ctx.clock) {
    ...
    for (var h = Math.ceil(from / 60); h * 60 <= to; h++) {
      ...
      if (slantW && horizontal) {
        var hA = spec.scale.at(h * 60), hHalf = lab.offsetWidth / 2, hReach = slantAt(cHour - rowH / 2);
        if (dayCuts.some(function (cut) { return hA > cut && hA - hHalf < cut + hReach + 2 * S; })) { lab.remove(); continue; }
      }
