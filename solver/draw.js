...
function draw(board, spec, ctx) {
  ...
  function nowNext(tb, dayIx, oneRow) {
    ...
    function line(ev) {
      return ev.title + ' ' + (ctx.clock ? ctx.clock(ev.start_min) : '');
    }
    var rows = [];
    if (on.length) rows.push([i18n.now || 'Now', on[0].title, badgesFor(on[0])]);
    if (later.length) {
      rows.push([i18n.next || 'Next', line(later[0]), badgesFor(later[0])]);
    } else if (!on.length) {
      ...
      if (tom.length) rows.push([i18n.tomorrow || 'Tomorrow', line(tom[0]), badgesFor(tom[0])]);
    }
