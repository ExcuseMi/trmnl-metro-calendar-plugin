      ...
      for (var pr = 0; pr < rows; pr++) {
        ...
        for (var pk = 0; pk < slides.length; pk++) {
          ...
          out.push({ want: want, a: pa, c: pc, side: pside, row: pr,
                     ...
                     railC: pside > 0 ? pill.c1 : pill.c0, pbox: pbox, beside: false, _bx: null });
        }
      }
    }
    ...
    for (var es = 0; es < 2; es++) {
      ...
      [pbox.c0, mid, pbox.c1 - want.h].forEach(function (ec, ei) {
        ...
        out.push({ want: want, a: ea, c: ec, side: 0, row: 0,
                   ...
                   railC: eside > 0 ? pill.c1 : pill.c0, pbox: pbox, beside: eside, _bx: null });
      });
    }
    return out;
  }
      ...
      for (var k = 0; k < sl.length; k++) {
        ...
        for (var ri = 0; ri < refs.length; ri++) {
          ...
          out.push({ want: want, a: a, c: c, side: side, row: r,
                     ...
                     slide: sl[k] * want.w, railC: railC, pbox: null, beside: false, _bx: null });
        }
      }
    }
  }
    ...
    [[wa0 - want.gap - want.w, c0s], [want.a1 + want.gap, c1s]].forEach(function (bs) {
      ...
      [bs[1] - want.h / 2, bs[1] - want.h + want.gap / 2, bs[1] - want.gap / 2].forEach(function (bc) {
        out.push({ want: want, a: bs[0], c: bc, side: 0, row: 0, off: 0, slide: 0,
                   railC: bs[1], pbox: null, beside: true, _bx: null });
      });
    });
  }
  return out;
...
// A CANDIDATE'S RECTANGLE, WORKED OUT ONCE.
//
// It is arithmetic on four numbers that never change for a given candidate --
// where it sits and how big its words are -- and it was being done afresh on
// every call, which on a hard board is millions of them, each handing back a
// new object for the collector to take away again. Kept on the candidate: the
// same answer, without the allocation.
//
// SAFE TO KEEP because the candidates a solve is handed are either built for
// it or come from the positions cache, whose key already contains the want's
// form, width, height and the pad around it (see `solve`). A candidate whose
// rectangle could differ is a candidate from a different entry.
//
// (Plain words for the two CSS properties either of those would otherwise
// name: this file is bundled into the template, and the plugin's linter
// counts a handful of property names in the markup it is given, comments
// included.)
function posBox(p) {
  return p._bx || (p._bx = { a0: p.a - p.want.pad, a1: p.a + p.want.w + p.want.pad,
...
function solve(wants, board, opts) {
  ...
  // THE ENVELOPE OF EACH CAPTION'S CANDIDATES, kept so that pairs which can
  // never touch are never priced. See `nbrs` below.
  var envs = new Array(n);
  var cands = wants.map(function (w, wIx) {
    ...
    // ...AND THE ENVELOPE WITH THEM. The reach of a caption's candidates is
    // arithmetic on exactly what the key above already names: the words, the
    // rows, the rail's own points. So it cannot have changed when they have
    // not, and walking every candidate again to work it out was the third
    // most expensive thing left in a solve -- 410ms of one hard board's three
    // seconds, paid on all eighty-seven thousand calls while the key itself
    // was hitting on ninety-five per cent of them. Read-only once built (the
    // three filters below only ask it questions), so one copy is safe to keep.
    var env = null;
    if (pEnt) { ps0 = pEnt.val.ps; env = pEnt.val.env; }
    if (!ps0) ps0 = positions(w, board, opts, own);
    if (!env) {
      env = { a0: w.a0 - w.w * 1.2 - w.pad - 4, a1: w.a1 + w.w * 1.2 + w.pad + 4,
      ...
      // The buffer still holds the key from above: nothing between here and
      // there touches it.
      if (cache) sigPut(sst.posTab, { ps: ps0, env: env });
    }
    envs[wIx] = env;
    var m = B.CAP_CLEAR + 1;
    var near = {
  ...
  // WHICH CAPTIONS CAN REACH EACH OTHER AT ALL.
  //
  // `pairCost` is zero for two captions that cannot touch, and almost every
  // pair on a board cannot: the greedy seed below priced every caption
  // against every caption placed before it, and `energy` against every
  // caption after it, so both were quadratic in a board's names while the
  // answer for most of the pairs was arithmetic on two rectangles that are
  // nowhere near each other. On the bench set that is the largest single
  // cost left in a solve.
  //
  // A caption's candidates all live inside one envelope (built above), so if
  // two envelopes cannot come within reach, NO pair of their candidates can,
  // whatever either one picks. That is a fact about the board rather than
  // about a placement, so it is worked out once per solve and the loops walk
  // the neighbours instead of the whole list.
  //
  // THE REACH IS NOT THE CLEARANCE. Two boxes that merely stand near each
  // other cost 30 (eight pixels), but a corridor's name in somebody else's
  // ROW is mistakable as far as four times a box's height away (`rowMate`),
  // and pruning at the clearance would silently stop charging for it. The
  // allowance is the larger, per pair, so this cannot change an answer: it can
  // only decline to ask a question whose answer is already known to be zero.
  var envs2 = envs;
  var nbrs = new Array(n);
  for (var ni = 0; ni < n; ni++) nbrs[ni] = [];
  for (var qi = 0; qi < n; qi++) {
    for (var qj = qi + 1; qj < n; qj++) {
      var ea = envs2[qi], eb = envs2[qj];
      var touch = true;
      if (ea && eb) {
        var mgn = 8 + 4 * Math.max(wants[qi].h || 0, wants[qj].h || 0);
        touch = !(ea.a1 + mgn < eb.a0 || eb.a1 + mgn < ea.a0
               || ea.c1 + 8 < eb.c0 || eb.c1 + 8 < ea.c0);
      }
      if (touch) { nbrs[qi].push(qj); nbrs[qj].push(qi); }
    }
  }
  ...
  function energy() {
    ...
    for (var i = 0; i < n; i++) {
      ...
      var nb = nbrs[i];
      for (var jx = 0; jx < nb.length; jx++) if (nb[jx] > i) e += pairCost(i, nb[jx]);
    }
    return e;
  }
  ...
  // Where each caption sits in that order, so the seed can ask its
  // neighbours which of them are already down without walking the list.
  var rank = new Array(n);
  for (var ri = 0; ri < n; ri++) rank[order[ri]] = ri;
  for (var oi = 0; oi < n; oi++) {
    ...
    for (var k = 0; k < cands[i].length; k++) {
      ...
      // ...against the ones already placed that can reach it. The rest are
      // zero by construction (see nbrs).
      for (var nj = 0; nj < nbi.length; nj++) if (rank[nbi[nj]] < oi) e += pairCost(i, nbi[nj]);
